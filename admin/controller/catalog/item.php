...
class ControllerCatalogItem extends Controller {
	...
	public function copy() {
		...
		if (isset($this->request->post['selected']) && $this->validateCopy()) {
			foreach ($this->request->post['selected'] as $item_id) {
				$this->model_catalog_item->copyitem($item_id);
			}
	...
	protected function getList() {
		...
		$data['copy'] = $this->url->link('catalog/item/copy', 'token=' . $this->session->data['token'] . $url, true);
		...
		foreach ($results as $result) {
			...
			$item_specials = $this->model_catalog_item->getitemSpecials($result['item_id']);

			foreach ($item_specials  as $item_special) {
				if (($itemitem_special['date_start'] == '0000-00-00' || strtotime($product_special['date_start']) < time()) && ($item_special['date_end'] == '0000-00-00' || strtotime($item_special['date_end']) > time())) {
					$special = $item_special['price'];
		...
		$data['sort_name'] = $this->url->link('catalog/item', 'token=' . $this->session->data['token'] . '&sort=pd.name' . $url, true);
		$data['sort_model'] = $this->url->link('catalog/item', 'token=' . $this->session->data['token'] . '&sort=p.model' . $url, true);
		$data['sort_price'] = $this->url->link('catalog/item', 'token=' . $this->session->data['token'] . '&sort=p.price' . $url, true);
		$data['sort_quantity'] = $this->url->link('catalog/item', 'token=' . $this->session->data['token'] . '&sort=p.quantity' . $url, true);
		$data['sort_status'] = $this->url->link('catalog/item', 'token=' . $this->session->data['token'] . '&sort=p.status' . $url, true);
		$data['sort_order'] = $this->url->link('catalog/item', 'token=' . $this->session->data['token'] . '&sort=p.sort_order' . $url, true);
	...
	protected function getForm() {
		...
		if (isset($this->request->post['item_description'])) {
			...
			$data['item_description'] = $this->model_catalog_item->getitemDescriptions($this->request->get['item_id']);
		} else {
		...
		if (isset($this->request->post['location'])) {
			...
			$data['location'] = $item_info['location'];
		} else {
			$data['location'] = '';
		}
		...
		if (isset($this->request->post['item_store'])) {
			$data['item_store'] = $this->request->post['item_store'];
		} elseif (isset($this->request->get['item_id'])) {
			$data['item_store'] = $this->model_catalog_item->getitemStores($this->request->get['item_id']);
		} else {
